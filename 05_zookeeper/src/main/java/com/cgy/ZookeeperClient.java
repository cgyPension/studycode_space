...
public class ZookeeperClient {
    ...
    public void create() throws KeeperException, InterruptedException {
        ...
        final String path = zooKeeperClient.create("/cgy", "shangguigu".getBytes(),
                ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        System.out.println(path);

        // 创建暂时节点
       /* final String paths = zooKeeperClient.create("/cgy", "hello".getBytes(),
    ...
    public void exist() throws KeeperException, InterruptedException {
        final Stat exists = zooKeeperClient.exists("/cgy", false);
    ...
    public void getAndWatch() throws KeeperException, InterruptedException {
        final Stat flag = zooKeeperClient.exists("/cgy", true);
        if(flag==null){
            System.out.println("节点不存在");
            return;
        }
        final byte[] data = zooKeeperClient.getData("/cgy", new Watcher() {
            @Override
            public void process(WatchedEvent watchedEvent) {
