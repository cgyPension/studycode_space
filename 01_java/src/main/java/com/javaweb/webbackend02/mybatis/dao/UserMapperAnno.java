...
public interface UserMapperAnno {
    ...
    @Results({ //代替的就是resultMap标签id标签 result标签
            ...
            @Result(property="ordersList",javaType = List.class,column = "id",many = @Many(select = "com.javaweb.webbackend02.mybatis.dao.OrdersMapperAnno.selectAllWhereUid")),
    })
    ...
    @Results({ //代替的就是resultMap标签id标签 result标签
            ...
            @Result(property="sex",column="sex"),
            @Result(property="address",column="address"),
            @Result(property="roleList",javaType = List.class,column = "id",many = @Many(select = "com.javaweb.webbackend02.mybatis.dao.RoleMapperAnno.selectUid")),
    })
